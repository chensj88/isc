...
$(function () {
    ...
    function initTreeView() {
        $.ajax({
            type: "POST",
            async: false,
            url: Common.getRootPath() + "/admin/ztree/moduleTree",
            cache:false,
            dataType: "json",
    ...
    $table.bootstrapTable({
        url: Common.getRootPath() + '/admin/role/list',// 要请求数据的文件路径
        height:600,
    ...
    $table.on('click', 'a[name="edit"]', function (e) {
        ...
        $.ajax({
            url: Common.getRootPath() + '/admin/role/getById',
            data: {'id': roleId},
            type: "post",
            dataType: 'json',
            async: false,
    ...
    $table.on('click', 'a[name="delete"]', function (e) {
        ...
        Ewin.confirm({message: "确认要删除["+roleName+"]的数据吗？"}).on(function (e) {
            ...
            $.ajax({
                type: "post",
                url: Common.getRootPath() + '/admin/role/deleteById',
                data: {"id": roleId},
                dataType: 'json',
    ...
    $table.on('click', 'a[name="tree"]', function (e) {
        ...
        $.ajax({
            type: "post",
            url: Common.getRootPath() + "/admin/rolemodule/query",
            dataType: "json",
            data:{'roleId':roleId},
            cache : false,
            async: false,
    ...
    $('#saveRole').on('click', function (e) {
        ...
        var url = '';
        if ($('#id').val().length == 0) {
            url = Common.getRootPath() + '/admin/role/add';
        } else {
            url = Common.getRootPath() + '/admin/role/update';
        }
