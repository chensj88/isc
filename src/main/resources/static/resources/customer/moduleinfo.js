...
$(function () {
    ...
    $table.bootstrapTable({
        url: Common.getRootPath() + '/admin/module/list',// 要请求数据的文件路径
        method: 'GET', // 请求方法
    ...
    function InitSubTable(index, row, $detail){
        ...
        $subTable.bootstrapTable({
            url: Common.getRootPath()+'/admin/module/childList',
            striped: true,
            method: 'GET', // 请求方法
    ...
    $table.on('click', 'a[name="edit"]', function (e) {
        ...
        var modId = $(this).attr('aid');
        $.ajax({
            url: Common.getRootPath() + '/admin/module/getById',
            data: {'modId': modId},
            type: "post",
            dataType: 'json',
            async: false,
    ...
    $table.on('click', 'a[name="delete"]', function (e) {
        ...
        Ewin.confirm({message: "确认要删除选择的数据吗？"}).on(function (e) {
            ...
            $.ajax({
                type: "post",
                url: Common.getRootPath() + '/admin/module/deleteById',
                data: {"modId": modId},
                dataType: 'json',
    ...
    $('#modPName').typeahead({
        source : function (query,process) {
            ...
            $.ajax({
                url : Common.getRootPath() + '/admin/module/queryModule',
                type: "post",
                dataType: 'json',
                async: false,
    ...
    $('#savemodule').on('click', function (e) {
        ...
        if ($('#modId').val().length == 0) {
            url = Common.getRootPath() + '/admin/module/add';
        } else {
            url = Common.getRootPath() + '/admin/module/update';
        }
