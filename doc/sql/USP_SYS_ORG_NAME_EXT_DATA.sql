/*
 机构全路径生成存储
*/
CREATE PROCEDURE USP_SYS_ORG_NAME_EXT_DATA
AS
  ...
  DECLARE cs CURSOR FOR
    ...
    FROM PUB_ORG T1
      LEFT JOIN PUB_ORG T2 ON T1.FID = T2.ID
      LEFT JOIN PUB_ORG T3 ON T2.FID = T3.ID
      LEFT JOIN PUB_ORG T4 ON T3.FID = T4.ID
      LEFT JOIN PUB_ORG T5 ON T4.FID = T5.ID
      LEFT JOIN PUB_ORG T6 ON T5.FID = T6.ID
      LEFT JOIN PUB_ORG T7 ON T6.FID = T7.ID
  BEGIN
    DELETE FROM PUB_ORG_EXT
    OPEN cs
    ...
    WHILE @@FETCH_STATUS = 0  --fetch语句执行成功返回0，fetch语句执行失败或者此行不在结果集中返回-1，被提取的行不存在则返回-2.
      BEGIN
        PRINT '====start insert===='
        INSERT INTO PUB_ORG_EXT VALUES( @orgid,@orgname,@orgcode,@orgnameext)
        PRINT '==== end  insert===='
