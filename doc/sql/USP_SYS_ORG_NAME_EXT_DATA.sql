USE [isc]
GO
/****** Object:  StoredProcedure [dbo].[USP_SYS_ORG_NAME_EXT_DATA]    Script Date: 2018/11/8 12:40:23 ******/
SET ANSI_NULLS ON
GO
...
/*
 机构全路径生成存储
*/
ALTER PROCEDURE [dbo].[USP_SYS_ORG_NAME_EXT_DATA]
AS
  ...
  DECLARE cs CURSOR FOR
    ...
    FROM SYS_ORG T1
           LEFT JOIN SYS_ORG T2 ON T1.FID = T2.ID
           LEFT JOIN SYS_ORG T3 ON T2.FID = T3.ID
           LEFT JOIN SYS_ORG T4 ON T3.FID = T4.ID
           LEFT JOIN SYS_ORG T5 ON T4.FID = T5.ID
           LEFT JOIN SYS_ORG T6 ON T5.FID = T6.ID
           LEFT JOIN SYS_ORG T7 ON T6.FID = T7.ID
  BEGIN
